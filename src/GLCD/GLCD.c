...
/**
* Passare il punto in alto a sinistra p0 = [x0,y0] p1 = [x1, y1]
*/
...
/**
*   Disegna un triangolo a partire dal 
*   @param: h = altezza e base triangolo, punto p0=x0,y0 che è il punto di partenza :
*		in alto a sinistra per dir='E'ST
*   in ___ per dir='W'EST
*		in ___ per dir='N'ORD
* 	in ___ per dir='S'UD
*/
void LCD_DrawTriangle( uint16_t x0, uint16_t y0, uint16_t h, uint16_t color, char dir ){
	int i,j;
	
	switch(dir) {
		case 'E':
			for(i=0,j=0; i<h; i+=2,j++){
				LCD_DrawLine(x0+i, y0+i-j,x0+i,y0+h-i+j,color);
				LCD_DrawLine(x0+i+1, y0+i-j,x0+i+1, y0+h-i+j,color);
			}
			LCD_DrawLine(x0+i, y0+i-j,x0+i,y0+h-i+j,color);
			break;
		case 'W':
		...
		case 'N':
			y0=y0+h;
			for(i=0,j=0; i<h; i+=2,j++){
				LCD_DrawLine(x0+i-j, y0-i,x0+h-i+j,y0-i,color);
				LCD_DrawLine(x0+i-j, y0-1-i,x0+h-i+j, y0-1-i,color);
			}
			LCD_DrawLine(x0+i-j, y0-i,x0+h-i+j,y0-i,color);
			break;
		case 'S':
			for(i=0,j=0; i<h; i+=2,j++){
				LCD_DrawLine(x0+i-j, y0+i,x0+h-i+j,y0+i,color);
				LCD_DrawLine(x0+i-j, y0+i+1,x0+h-i+j, y0+i+1,color);
			}
			LCD_DrawLine(x0+i-j, y0+i,x0+h-i+j,y0+i,color);
			break;
	}
